...
- 当前正式版本为 `0.2.0`；生产安装固定使用 `v0.2.0` tag，不跟随漂移的 `main`。
...
安装时，把下面内容复制给 Codex、Claude Code、Cursor、GitHub Copilot 或其他支持 Agent Skills 的宿主：

```text
请检查并安装这个 Skill：

https://github.com/KelvinWu/hk-pre-owned-rolex-monitoring/tree/v0.2.0/skills/hk-pre-owned-rolex-monitoring
...
生产安装固定使用 `v0.2.0` Release 的 wheel 和 `SHA256SUMS.txt`；需要回滚时可使用 `v0.1.1`。
...
1. 为 `--state-dir` 或 `INVENTORY_SENTINEL_HOME` 选择跨任务持久目录。
2. 运行 `monitor init --output <manifest.yaml>` 生成安全草稿；默认不会静默创建定时任务。
3. 确认时区、运行时间和通知目标后运行 `monitor create`，再使用两份一致的成功快照建立基线。
4. 使用 `monitor history`、`monitor show-run` 和 `report build` 回看历史，不用重新抓取代替昨天记录。
5. 只有需要行情时才运行 `market sources` 和来源诊断，再用正式 API 或 Market Packet 构建命令准备证据。
6. 生成 Runtime Plan；定时和通知由宿主执行，Skill 不伪造任务 ID 或发送结果。
